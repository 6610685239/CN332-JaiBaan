...
const SUPABASE_BUCKET = process.env.SUPABASE_FACILITY_BUCKET || 'Facility_images'
...
const saveFacilityPhoto = async (file) => {
...
const deleteFacilityPhoto = async (filename) => {
  try {
    const { error } = await supabase.storage.from(SUPABASE_BUCKET).remove([filename])
    if (error) console.error('Supabase delete error:', error)
  } catch (err) {
    console.error('Failed to delete facility photo from Supabase:', err)
  }
}

module.exports = { saveFacilityPhoto, deleteFacilityPhoto }
